...
class StockPriceGraph():
    ...
    def draw_html(self):
        ...
        self.data['Status'] = [status(o, c) for o, c in zip(self.data.Open, self.data.Close)]
        ...
        fig.segment(self.data.index, self.data.High, self.data.index, self.data.Low, color='black')

        # draw green candles then
        fig.rect(self.data.index[self.data.Status == 'green'], 
